...
// menu function that also sets up the two subwindows
void mymenu(int value) {
    if (value == 1) {
...
void draw_mergesort_window(void) {
    ...
    for (double w = 0.0; w < win_width; w = w + factor) {
        ...
        int col_height = upper_win_height * (xs_mergesort[(int)pos] / (double)xs_highest); 
        for (int h = 0; h < col_height; h++) {
            int rounded_w = (int)w;
            fb[h][rounded_w][0] = 255;
        }
    }
    glWindowPos2s(0, 0) ;
...
void draw_quicksort_window(void) {
    ...
    for (double w = 0.0; w < win_width; w = w + factor) {
        double pos = w / factor; // will always be an integer
        int col_height = lower_win_height * (xs_quicksort[(int)pos] / (double)xs_highest); 
        for (int h = 0; h < col_height; h++) {
            int rounded_w = (int)w;
            fb[h][rounded_w][0] = 255;
        }
...
void update_lists(void) {
    // mergesort part
    // do one merge, then glutSetWindow(upper_win), then redisplay
    bool performed_a_merge = false;
    
    // merged node should have info as to where it's actually merging stuff.
    ...
    if (performed_a_merge) {
        ...
        // quicksort part
        // we want to redisplay at every partition, which is every step.
